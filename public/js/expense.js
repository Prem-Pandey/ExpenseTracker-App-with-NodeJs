...
console.log("token at third line: "+token)
let balance = 0;
let EXPENSE_ID = null;
...
function deleteExpense(li){
    ...
    axios.delete(`${HOST}/expense/delete-expense/${expenseId}`, {headers: {Authorization : `Bearer ${token}`}})
    .then((res) => {
...
function editExpense(li){
    const expenseId = li.id;
    axios.get(`${HOST}/expense/edit-expense/${expenseId}`, {headers: {Authorization : `Bearer ${token}`}})
    .then((res) => {
...
async function getAllExpenses(page=1, limit=5){
    ...
    // const tokena = await parseJwt(token)
    const token = localStorage.getItem("tokenaaaaa");
    console.log("tokenaaa:>>"+token)
    console.log("token:>>"+token)
    // const decodedToken = jwt.verify(token, '123@key');
    //     const userId = decodedToken.id;
    //     console.log("tokenaaaa:>>>>>>"+ decodedToken)
    //     console.log("userId:>>>>>>"+userId);
    // const decodedToken = parseJwt(token);
    console.log("token in get expense: "+ token)
    await axios.get(`${HOST}/expense/get-expenses?page=${page}&limit=${limit}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("tokenaaaaa")}`
        }
      })
    .then((res) => {
...
function addExpense(e){ 
    ...
    // const decodedToken = parseJwt(token);
    // const tokena = "123"
    axios.post(`${HOST}/expense/add-expense`, expense, {headers: {Authorization: `Bearer ${localStorage.getItem("tokenaaaaa")}`}})
    .then((res) => {
...
function razorpay(e){
    axios.get(`${HOST}/purchase/premium-membership`, {headers: {Authorization: `Bearer ${localStorage.getItem("tokenaaaaa")}`}})
    .then((res) => {
        const options = {
            ...
            "handler": async function () {
                try{
                    ...
                    }, {headers: {Authorization: `Bearer ${localStorage.getItem("tokenaaaaa")}`} });
...
function showUserInfoInDOM(){
    ...
    console.log("is premius: "+isPremiumUser)
